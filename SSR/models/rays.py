...
def get_rays_camera(B, H, W, fx, fy, cx, cy, depth_type, convention="opencv"):
    ...
    i, j = torch.meshgrid(
        torch.arange(W), torch.arange(H), indexing='ij'
...
def get_rays_camera_np(B, H, W, fx, fy, cx, cy, depth_type, convention="opencv"):
    ...
    i, j = np.meshgrid(
        np.arange(W, dtype=np.float32), np.arange(H, dtype=np.float32), indexing="xy"
    )  # pytorch's meshgrid has default indexing='ij'
...
def get_rays_world_np(T_WC, dirs_C):
    R_WC = T_WC[:, :3, :3]  # Bx3x3
    dirs_W = (R_WC * dirs_C[..., np.newaxis, :]).sum(
        axis=-1
    )  # dot product, equals to: [c2w.dot(dir) for dir in dirs]
...
def stratified_bins(min_depth, max_depth, n_bins, n_rays, device):
