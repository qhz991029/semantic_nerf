...
def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024 * 64):
    ...
    )  # a list of points to query
    embedded = embed_fn(inputs_flat)  # embed coordinates

    if viewdirs is not None:
        input_dirs = viewdirs[:, None].expand(
            inputs.shape
        )  # all samples on a ray share the same view dir
        input_dirs_flat = torch.reshape(input_dirs, [-1, input_dirs.shape[-1]])
        embedded_dirs = embeddirs_fn(input_dirs_flat)
        embedded = torch.cat([embedded, embedded_dirs], -1)

    outputs_flat = batchify(fn, netchunk)(embedded)  # [num_rays * num_samples, 32]
    outputs = torch.reshape(
        outputs_flat, list(inputs.shape[:-1]) + [outputs_flat.shape[-1]]
    )  # [num_rays, num_samples, 32]
    return outputs
...
def raw2outputs(
    ...
    Args:
        raw: [num_rays, num_samples along ray, 3 (rgb) + 1 (alpha) + 28 (num_classes)]. Prediction from model.
    ...
    surface_normal = F.normalize(raw[..., 4:7], dim=-1)  # [N_rays, N_samples, 3]

    if raw_noise_std > 0.0:
        noise = torch.randn(raw[..., 3].shape) * raw_noise_std  # a noise added to alpha
        noise = noise.cuda()
    else:
        noise = 0.0
    ...
    # all the tasks share the same weights
    rgb_map = torch.sum(weights[..., None] * rgb, -2)  # [N_rays, 3]
    # [N_rays, 3], the accumulated opacity along the rays, equals "1 - (1-a1)(1-a2)...(1-an)" mathematically
    surface_normal_map = F.normalize(
        torch.sum(weights[..., None] * surface_normal, -2), dim=-1
    )
    ...
    # )
    disp_map = torch.zeros_like(depth_map)
