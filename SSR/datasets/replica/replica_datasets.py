...
class ReplicaDatasetCache(Dataset):
    def __init__(self, data_dir, train_ids, test_ids, img_h=None, img_w=None):
        ...
        self.surface_normal_dir = os.path.join(data_dir, "surface_normal")
        ...
        if self.semantic_instance_dir is not None:
            self.instance_list = sorted(
                glob.glob(self.semantic_instance_dir + "/semantic_instance_*.png"),
                key=lambda file_name: int(file_name.split("_")[-1][:-4]),
            )
        # whether enable_multitask or not, semantic will be included
        self.train_samples = {
            "image": [],
            "depth": [],
            "surface_normal": [],
        ...
        for idx in train_ids:
            image = (
                cv2.imread(self.rgb_list[idx])[:, :, ::-1] / 255.0
            )  # change from BGR uint8 to RGB float
            depth = (
            ...
            if self.semantic_instance_dir is not None:
                instance = cv2.imread(
                    self.instance_list[idx], cv2.IMREAD_UNCHANGED
                )  # uint16
            ...
            if (self.img_h is not None and self.img_h != image.shape[0]) or (
                ...
                surface_normal = surface_normal.permute(2, 0, 1).unsqueeze(0)
                surface_normal = F.interpolate(surface_normal, [self.img_h, self.img_w])
                surface_normal = surface_normal.squeeze().permute(1, 2, 0)
                semantic = cv2.resize(
            ...
            self.train_samples["surface_normal"].append(surface_normal)
        ...
        for idx in test_ids:
            ...
            self.test_samples["surface_normal"].append(surface_normal)
        ...
        for key in self.train_samples.keys():
            print(
                "{} has shape of {}, type {}.".format(
                    key, self.train_samples[key].shape, self.train_samples[key].dtype
                )
            )
        print()
        ...
        for key in self.test_samples.keys():
            print(
                "{} has shape of {}, type {}.".format(
                    key, self.test_samples[key].shape, self.test_samples[key].dtype
                )
            )

    def sample_label_maps(
        self, sparse_ratio=0.5, K=0, random_sample=False, load_saved=False
    ):
        """
        ...
        if load_saved is False:
            if K == 0:
                K = int(
                    self.train_num * sparse_ratio
                )  # number of skipped training frames, mask=0

            N = self.train_num - K  # number of used training frames,  mask=1
            assert (
                np.sum(self.mask_ids) == self.train_num
            )  # sanity check that all masks are avaible before sampling
            ...
            if random_sample:
                ...
                if sparse_ratio <= 0.5:  # skip less/equal than half frames
                    ...
                    self.mask_ids = np.zeros_like(
                        self.mask_ids
                    )  # disable all images and  evenly enable N images in total
                    q, r = divmod(self.train_num, N)
                    indices = [q * i + min(i, r) for i in range(N)]
                    self.mask_ids[indices] = 1

            print(
                "{} of {} semantic labels are sampled (sparse ratio: {}).".format(
                    sum(self.mask_ids), len(self.mask_ids), sparse_ratio
                )
            )
            noisy_sem_dir = os.path.join(
                self.semantic_class_dir, "noisy_pixel_sems_sr{}".format(sparse_ratio)
            )
    ...
    def add_pixel_wise_noise_label(
        self,
        sparse_views=False,
        sparse_ratio=0.0,
        random_sample=False,
        ...
        if not load_saved:
            if sparse_views:
                self.sample_label_maps(
                    sparse_ratio=sparse_ratio, random_sample=random_sample
                )
            ...
            for i in range(len(self.mask_ids)):
                if (
                    self.mask_ids[i] == 1
                ):  # add label noise to unmasked/available labels
                    noisy_index_1d = np.random.permutation(num_pixel)[:num_pixel_noisy]
                    faltten_sem = train_sem[i].flatten()

                    faltten_sem[noisy_index_1d] = np.random.choice(
                        self.num_semantic_class, num_pixel_noisy
                    )
            ...
            print(
                "{} of {} semantic labels are added noise {} percent area ratio.".format(
                    sum(self.mask_ids), len(self.mask_ids), noise_ratio
                )
            )

            if visualise_save:
                noisy_sem_dir = os.path.join(
                    self.semantic_class_dir,
                ...
                semantic_remap_clean = self.train_samples[
                    "semantic_remap_clean"
                ]  # [H, W, 3]

                # save semantic labels
                for i in range(len(self.mask_ids)):
                    if self.mask_ids[i] == 1:
                        vis_noisy_semantic = colour_map_np[
                            semantic_remap[i]
                        ]  # [H, W, 3]
                        vis_semantic_clean = colour_map_np[
                            semantic_remap_clean[i]
                        ]  # [H, W, 3]

                        imageio.imwrite(
                            os.path.join(
                                noisy_sem_dir, "semantic_class_{}.png".format(i)
                            ),
                            semantic_remap[i],
                        )
                        imageio.imwrite(
                            os.path.join(
                                noisy_sem_dir, "vis_sem_class_{}.png".format(i)
                            ),
                            vis_noisy_semantic,
                        )
                        ...
                        assert np.all(
                            vis_noisy_semantic == vis_semantic_clean
                        )  # apply this check to skipped frames
                ...
                imageio.mimwrite(
                    os.path.join(
                        noisy_sem_dir, "noisy_sem_ratio_{}.mp4".format(noise_ratio)
                    ),
                    np.stack(vis_noisy_semantic_list, 0),
                    fps=30,
                    quality=8,
                )

                imageio.mimwrite(
                    os.path.join(noisy_sem_dir, "clean_sem.mp4"),
                    np.stack(vis_semantic_clean_list, 0),
                    fps=30,
                    quality=8,
                )
        else:
            ...
            self.mask_ids = np.load(os.path.join(noisy_sem_dir, "mask_ids.npy"))
            semantic_img_list = []
            semantic_path_list = sorted(
                glob.glob(noisy_sem_dir + "/semantic_class_*.png"),
                key=lambda file_name: int(file_name.split("_")[-1][:-4]),
            )
    ...
    def add_instance_wise_noise_label(
        self,
        sparse_views=False,
        sparse_ratio=0.0,
        random_sample=False,
        ...
        if not load_saved:
            ...
            for img_idx in range(self.train_num):
                ...
                for ins_idx in instance_id:
                    ...
                    if (
                        instance_ratio > 0 and self.mask_ids[img_idx] == 1
                    ):  # larger than 1% image area and the image is also sampled into training set
                        instance_maps_dict[ins_idx].append([img_idx, instance_ratio])

            num_frame_per_instance_id = np.asarray(
                [len(x) for x in instance_maps_dict.values()]
            )
            num_flip_frame_per_instance_id = (
                num_frame_per_instance_id * flip_ratio
            ).astype(np.int)

            for k, v in instance_maps_dict.items():
                instance_maps_dict[k] = sorted(
                    instance_maps_dict[k], key=lambda x: x[1]
            ...
            if not uniform_flip:
                ...
                for i in range(len(instance_id)):  # loop over instance id
                    selected_frame_id = [
                        x[0]
                        for x in instance_maps_dict[instance_id[i]][
                            : num_flip_frame_per_instance_id[i]
                        ]
                    ]
                    for (
                        m
                        ...
                        selected_frame_id
                    ):  # loop over image ids having the selected instance
                        self.train_samples["semantic_remap"][m][
                            self.train_samples["instance"][m] == instance_id[i]
                        ] = np.random.choice(self.num_semantic_class, 1)
            else:
                if flip_ratio <= 0.5:  # flip less/equal than half frames
                    for i in range(len(instance_id)):  # loop over instance id
                        ...
                        valid_frame_id_list = [
                            x[0] for x in instance_maps_dict[instance_id[i]]
                        ]
                        selected_frame_id = [
                            valid_frame_id_list[flip_id] for flip_id in indices_to_flip
                        ]
                        for (
                            m
                        ) in (
                    ...
                    for i in range(len(instance_id)):  # loop over instance id
                        ...
                        indices_to_flip = [
                            x
                            for x in range(num_frame_per_instance_id[i])
                            if x not in indices_NOT_flip
                        ]
                        valid_frame_id_list = [
                            x[0] for x in instance_maps_dict[instance_id[i]]
                        ]
            ...
            colour_map_np = self.colour_map_np
            vis_flip_semantic = [
                colour_map_np[sem] for sem in self.train_samples["semantic_remap"]
            ]
            vis_gt_semantic = [
                colour_map_np[sem] for sem in self.train_samples["semantic_remap_clean"]
            ]

            if visualise_save:
                flip_sem_dir = os.path.join(
                    self.semantic_class_dir, "flipped_chair_nr_{}".format(flip_ratio)
                )
                ...
                for i in range(len(vis_flip_semantic)):
                    imageio.imwrite(
                        os.path.join(flip_sem_dir, "semantic_class_{}.png".format(i)),
                        self.train_samples["semantic_remap"][i],
                    )
                    imageio.imwrite(
                        os.path.join(flip_sem_dir, "vis_sem_class_{}.png".format(i)),
                        vis_flip_semantic[i],
                    )
                    imageio.imwrite(
                        os.path.join(flip_sem_dir, "vis_gt_{}.png".format(i)),
                        vis_gt_semantic[i],
                    )
        else:
            print("Load saved noisy labels.")
            ...
            semantic_img_list = []
            semantic_path_list = sorted(
                glob.glob(flip_sem_dir + "/semantic_class_*.png"),
                key=lambda file_name: int(file_name.split("_")[-1][:-4]),
            )
            assert len(semantic_path_list) > 0
            for idx in range(len(self.mask_ids)):
    ...
    def super_resolve_label(self, down_scale_factor=8, dense_supervision=True):
        ...
        if (
            dense_supervision
        ):  # for dense labelling,  we down-scale and up-scale label maps again
            scaled_low_res_train_label = []
            for i in range(self.train_num):
                low_res_label = cv2.resize(
                    self.train_samples["semantic_remap"][i],
                    (self.img_w // down_scale_factor, self.img_h // down_scale_factor),
                    interpolation=cv2.INTER_NEAREST,
                )

                scaled_low_res_label = cv2.resize(
                    low_res_label,
                    (self.img_w, self.img_h),
                    interpolation=cv2.INTER_NEAREST,
                )
            ...
            self.train_samples["semantic_remap"] = (
                self.train_samples["semantic_remap"]
                * valid_low_res_pixel_mask[None, ...]
            ).astype(np.uint8)
            # we mask all the decimated pixel label to void class==0

    def simulate_user_click_partial(
        self, perc=0, load_saved=False, visualise_save=True
    ):
        """
        ...
        single_click = (
            True if perc == 0 else False
        )  # single_click: whether to use single click only from each class
        ...
        if not load_saved:
            ...
            if single_click:
                ...
                for i in range(self.train_num):
                    if (i + 1) % 10 == 0:
                        print(
                            "Generating partial label of ratio {} for frame {}/{}.".format(
                                perc, i, self.train_num
                            )
                        )
                    ...
                    for l in valid_class:
                        ...
                        sample_ind = np.random.choice(
                            label_idx.shape[0], 1, replace=False
                        )
                ...
                print(
                    "Partial Label images with centroid sampling (extreme) has completed."
                )
                ...
                for i in range(self.train_num):
                    ...
                    im = self.train_samples["semantic_remap"][i]
                    void_class = [0]
                    label_class = np.unique(
                        im
                    ).tolist()  # find the unique class-ids in the current training label
                    ...
                    for l in valid_class:
                        ...
                        target_num = int(
                            perc * label_mask_.sum()
                        )  # find the target and total number of pixels belong to class-l in the current image
                        label_mask[
                            label_idx_[0, 0], label_idx_[0, 1]
                        ] = 1  # full-zero mask with only selected pixel to be 1
                        ...
                        while label_mask_true.sum() < target_num:
                            ...
                            if num_after_grow == num_before_grow:
                                ...
                                uncovered_region_mask = (
                                    label_mask_ - label_mask_true
                                )  # pixels which are equal to 1 are un-sampled regions and belong to current class
                                label_idx = np.transpose(
                                    np.nonzero(uncovered_region_mask)
                                )  # Nx2
                                sample_ind = np.random.choice(
                                    label_idx.shape[0], 1, replace=False
                                )  # shape [1,]
            ...
            if visualise_save:
                partial_sem_dir = os.path.join(
                    self.semantic_class_dir, "partial_perc_{}".format(perc)
                )
                ...
                vis_partial_sem = []
                for i in range(self.train_num):
                    vis_partial_semantic = colour_map_np[
                        self.train_samples["semantic_remap"][i]
                    ]  # [H, W, 3]
                    imageio.imwrite(
                        os.path.join(
                            partial_sem_dir, "semantic_class_{}.png".format(i)
                        ),
                        self.train_samples["semantic_remap"][i],
                    )
                    imageio.imwrite(
                        os.path.join(partial_sem_dir, "vis_sem_class_{}.png".format(i)),
                        vis_partial_semantic,
                    )
                    vis_partial_sem.append(vis_partial_semantic)

                imageio.mimwrite(
                    os.path.join(partial_sem_dir, "partial_sem.mp4"),
                    self.train_samples["semantic_remap"],
                    fps=30,
                    quality=8,
                )
                imageio.mimwrite(
                    os.path.join(partial_sem_dir, "vis_partial_sem.mp4"),
                    np.stack(vis_partial_sem, 0),
                    fps=30,
                    quality=8,
                )

        else:  # load saved single-click/partial semantics
            saved_partial_sem_dir = os.path.join(
                self.semantic_class_dir, "partial_perc_{}".format(perc)
            )
            semantic_img_list = []
            semantic_path_list = sorted(
                glob.glob(saved_partial_sem_dir + "/semantic_class_*.png"),
                key=lambda file_name: int(file_name.split("_")[-1][:-4]),
            )
            assert len(semantic_path_list) > 0
            for idx in range(self.train_num):
                semantic = imread(semantic_path_list[idx])
                semantic_img_list.append(semantic)
            self.train_samples["semantic_remap"] = np.asarray(semantic_img_list).astype(
                np.uint8
            )
